...
#===============================================================================
#
#   GPT4Free - Automated Installation Script
...
show_usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  domain_name    The domain name for the GPT4Free web interface (e.g., gpt.example.com)"
    echo ""
    echo "Example:"
    echo "  $0 gpt.example.com"
    echo ""
...
validate_domain() {
    ...
    if [[ ! "$domain" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
        ...
        print_info "Please enter a valid domain (e.g., gpt.example.com)"
        exit 1
    fi
}
...
show_banner() {
    ...
    echo -e "${CYAN}   |${NC}   ${BOLD}${WHITE}GPT4Free${NC}                                                            ${CYAN}|${NC}"
...
install_dependencies() {
    ...
    print_step "Installing Chromium browser (for browser automation)..."
    apt-get install -y -qq chromium-browser > /dev/null 2>&1 || apt-get install -y -qq chromium > /dev/null 2>&1 || print_warning "Chromium installation skipped"
...
setup_python_environment() {
    ...
    print_step "Installing/updating gpt4free dependencies (this may take a few minutes)..."
    su - "$CURRENT_USER" -c "cd '$INSTALL_DIR' && source '$VENV_DIR/bin/activate' && pip install -r requirements.txt" > /dev/null 2>&1
    print_success "Dependencies installed"

    print_step "Installing gpt4free package..."
    su - "$CURRENT_USER" -c "cd '$INSTALL_DIR' && source '$VENV_DIR/bin/activate' && pip install -e ." > /dev/null 2>&1
    print_success "GPT4Free package installed"
}
...
[Unit]
Description=GPT4Free Web Interface and API
...
ExecStart=$VENV_DIR/bin/python -m g4f.cli gui --port $APP_PORT
Restart=always
RestartSec=10
...
show_completion_message() {
    ...
    echo -e "  ${CYAN}*${NC} Web UI:        ${BOLD}https://$DOMAIN_NAME/chat/${NC}"
